...
- Hermes plugin action audit logging lives in `hermes-clicky-plugin/audit_log.py`; it is append-only JSONL, redacts credentials/full typed text by default, and supports disabled/minimized modes for privacy.
- Hermes post-action verification lives in `hermes-clicky-plugin/post_action_verification.py`; keep it deterministic through injected observation seams and stop automation on failed/uncertain/blockedByPrompt outcomes instead of guessing.
- Hermes scoped autopilot policy lives in `hermes-clicky-plugin/scoped_autopilot.py`; keep it as deterministic session/gating data only. It must group audit records by `sessionId`, route every step through permission and safety decisions, and stop on timeout/max-steps/scope violations/blocked actions/failed verification/user cancel without executing desktop actions itself.
